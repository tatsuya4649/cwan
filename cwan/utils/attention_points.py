...
def to_attention_points(attention_maps,beta=20):
    ...
    for n in range(attention_maps.shape[0]):
        ...
        ones = torch.ones(binary_point.shape).to(device)
        zeros = torch.zeros(binary_point.shape).to(device)
        ...
        if one_count > beta:
            ...
            cat = cat.tolist()
            binary_point_np = np.zeros_like(np_binary)
            for i in cat:
                j,k = i
                binary_point_np[j][k] = 1
            binary_point = torch.from_numpy(binary_point_np).to(device)
